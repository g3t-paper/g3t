...
from vggt_long.LoopModels.vpr_model import VPRModel
...
class LoopDetector:
    ...
    def __init__(self, image_dir,
        ...
        Args:
            ...
            verbose: Whether to print progress/debug messages
        """
        self.config = config
    ...
    def extract_descriptors(self):
        ...
        for i in tqdm(range(0, len(self.image_paths), self.batch_size), desc="Extracting features", disable=not self.verbose):  # noqa
            ...
            for path in batch_paths:
                try:
                    ...
                    img = torch.zeros(3, 224, 224) if self.image_size is None else torch.zeros(3, self.image_size[0], self.image_size[1])  # noqa
                    batch_imgs.append(img)
    ...
    def find_loop_closures(self):
        ...
        similarities, indices = faiss_index.search(normalized_descriptors, self.top_k + 1)
...
def main():
    ...
    parser.add_argument(
        "--image_size", nargs=2, type=int, default=[336, 336],
    ...
    parser.add_argument(
        "--use_nms", action="store_true", default=True,
        help="Whether to use Non-Maximum Suppression (NMS) filtering"
    )
    parser.add_argument(
        "--nms_threshold", type=int, default=25,
