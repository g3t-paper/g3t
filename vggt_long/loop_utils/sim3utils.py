...
def align_point_maps(point_map1, conf1, point_map2, conf2, conf_threshold, verbose=False):
...
def compute_alignment_error(point_map1, conf1, point_map2, conf2, conf_threshold, s, R, t, verbose=False):
    """
...
def save_confident_pointcloud(points, colors, confs, output_path, conf_threshold, sample_ratio=1.0, verbose=False):
    """
...
def save_confident_pointcloud_batch(
    points, colors, confs, output_path, conf_threshold,
    sample_ratio=1.0, batch_size=1000000
):
    """
...
def save_pointcloud(points, colors, output_path):
    """
    Saves all points and colors regardless of confidence, with batch writing to handle large point clouds.
...
def process_loop_list(chunk_index, loop_list, half_window=10, verbose=False):
    """
...
def merge_ply_files(input_dir, output_path, verbose=False):
    """
...
def _expand_1DoF_Ry(R):
    """
    Expands a 2x2 rotation matrix to a 1 DoF 3x3 rotation matrix around the Y axis.
    """
...
def weighted_estimate_se3(source_points, target_points, weights, rotation_dof=3):
    """
    source_points:  (Nx3)
    target_points:  (Nx3)
    :weights:  (N,) [0,1]
    ...
    if rotation_dof == 1:
        # projecting points to the XZ plane
        weighted_src = np.concatenate(
            (weighted_src[:, 0:1], weighted_src[:, 2:3]), axis=1
        )  # (N, 2)
        weighted_tgt = np.concatenate(
            (weighted_tgt[:, 0:1], weighted_tgt[:, 2:3]), axis=1
        )  # (N, 2)

    elif rotation_dof == 3:
        pass  # nothing to be done in this case
    else:
        raise ValueError("Unsupported rotation_dof. Must be 1 or 3.")
...
def weighted_estimate_sim3(source_points, target_points, weights, rotation_dof=3):
    """
    source_points:  (Nx3)
    target_points:  (Nx3)
    :weights:  (N,) [0,1]
...
def robust_weighted_estimate_sim3(
    src, tgt, init_weights, delta=0.1, max_iters=20,
    tol=1e-9, using_sim3=True, rotation_dof=3, verbose=False
):
    """
    src:  (Nx3)
    tgt:  (Nx3)
    ...
    """
    if using_sim3:
        s, R, t = weighted_estimate_sim3(src, tgt, init_weights, rotation_dof=rotation_dof)
    else:
        s, R, t = weighted_estimate_se3(src, tgt, init_weights, rotation_dof=rotation_dof)
    ...
    for iter in range(max_iters):
        ...
        if using_sim3:
            s_new, R_new, t_new = weighted_estimate_sim3(src, tgt, combined_weights, rotation_dof=rotation_dof)
        else:
            s_new, R_new, t_new = weighted_estimate_se3(src, tgt, combined_weights, rotation_dof=rotation_dof)
...
@njit(cache=True)
def _weighted_estimate_se3_numba(source_points, target_points, weights, rotation_dof=3):
    # Ensure float32
    ...
    if total_weight < 1e-6:
        ...
        if rotation_dof == 1:
            dummy_H = np.zeros((2, 2), dtype=np.float32)
        elif rotation_dof == 3:
            dummy_H = np.zeros((3, 3), dtype=np.float32)
        else:
            raise ValueError("Unsupported rotation_dof. Must be 1 or 3.")

        return 1.0, np.zeros(3, dtype=np.float32), np.zeros(3, dtype=np.float32), dummy_H
...
@njit(cache=True)
def _weighted_estimate_sim3_numba(source_points, target_points, weights, rotation_dof=3):
    # Ensure float32
    source_points = source_points.astype(np.float32)
    target_points = target_points.astype(np.float32)
    weights = weights.astype(np.float32)
...
def weighted_estimate_sim3_numba(source_points, target_points, weights, using_sim3=True, rotation_dof=3):
    if using_sim3:
        s, mu_src, mu_tgt, H = _weighted_estimate_sim3_numba(
            source_points, target_points, weights, rotation_dof=rotation_dof
        )
    else:
...
def robust_weighted_estimate_sim3_numba(
    ...
    s, R, t = weighted_estimate_sim3_numba(
        src, tgt, init_weights, using_sim3=using_sim3, rotation_dof=rotation_dof
    )
    ...
    for iter in range(max_iters):
        ...
        s_new, R_new, t_new = weighted_estimate_sim3_numba(
            src, tgt, combined_weights, using_sim3=using_sim3, rotation_dof=rotation_dof
        )
...
def warmup_numba(verbose=False):
    ...
    try:
        _ = _weighted_estimate_sim3_numba(src, tgt, weights, rotation_dof=3)
        _ = _weighted_estimate_sim3_numba(src, tgt, weights, rotation_dof=1)
        if verbose:
    ...
    try:
        _ = _weighted_estimate_se3_numba(src, tgt, weights, rotation_dof=3)
        _ = _weighted_estimate_se3_numba(src, tgt, weights, rotation_dof=1)
        if verbose:
...
def weighted_align_point_maps(point_map1, conf1, point_map2, conf2, mask, conf_threshold, config, rotation_dof=3, verbose=False):  # noqa
    """ point_map2 -> point_map1"""
    b1, _, _, _ = point_map1.shape
    b2, _, _, _ = point_map2.shape
    b = min(b1, b2)
