...
from train_utils.normalization import normalize_gravity_camera_extrinsics_and_points_batch
...
class MultitaskLoss(torch.nn.Module):
    ...
    """
    def __init__(self, camera=None, depth=None, point=None, track=None, gravity_camera=None, **kwargs):
        super().__init__()
        # Loss configuration dictionaries for each task
        self.camera = camera
        self.gravity_camera = gravity_camera
        self.depth = depth
        self.point = point
        self.track = track
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        # normalize gravity-aligned camera extrinsics and points
        (
            normalized_g2cs, normalized_w2gs, normalized_cam_points,
            normalized_world_points, normalized_depths
        ) = \
            normalize_gravity_camera_extrinsics_and_points_batch(
                c2gs = batch["c2gs"],
                g2ws=batch["g2ws"],
        ...
        batch["g2cs"] = normalized_g2cs
        batch["w2gs"] = normalized_w2gs
        ...
        # Gravity-aligned camera pose loss - if local and global pose encodings are predicted
        if "local_pose_enc_list" in predictions and "global_pose_enc_list" in predictions:
            camera_loss_dict = compute_gravity_camera_loss(predictions, batch, **self.gravity_camera)

            local_camera_loss = camera_loss_dict["loss_local_camera"] * self.gravity_camera["weight"]
            global_camera_loss = camera_loss_dict["loss_global_camera"] * self.gravity_camera["weight"]
            total_loss = total_loss + local_camera_loss + global_camera_loss
        ...
        if "depth" in predictions:
            ...
            depth_loss = depth_loss_dict["loss_conf_depth"] + depth_loss_dict["loss_reg_depth"] \
                + depth_loss_dict["loss_grad_depth"]
            depth_loss = depth_loss * self.depth["weight"]
            total_loss = total_loss + depth_loss
            loss_dict.update(depth_loss_dict)

        # 3D point reconstruction loss - if gravity points are predicted
        if "world_points" in predictions:
            point_loss_dict = compute_point_loss(predictions, batch, **self.point)
            point_loss = point_loss_dict["loss_conf_point"] + point_loss_dict["loss_reg_point"] \
...
def _normalize_points(points, mask):
    """
    Normalize points similar to DUSt3R

    Args:
        points: (B, S, H, W, 3) point cloud
        mask: (B, S, H, W) valid mask

    Returns:
        normalized_points: (B, S, H, W, 3) normalized point cloud
    """
    ...
    normalized_points = points / avg_scale.view(-1, 1, 1, 1, 1)

    return normalized_points


def _gravity_camera_loss_helper(
    key, pred_pose_encodings, gt_pose_encoding, valid_frame_mask,
    n_stages, loss_type, gamma, weight_trans, weight_rot, weight_focal,
    pose_encoding_type
):
    ...
    for stage_idx in range(n_stages):
        ...
        if valid_frame_mask.sum() == 0:
            ...
            loss_T_stage, loss_R_stage, loss_FL_stage = camera_loss_single(
                ...
                loss_type=loss_type, pose_encoding_type=pose_encoding_type,
            )
    ...
    loss_dict = {
        f"loss_{key}_camera": total_camera_loss,
        f"loss_{key}_T": avg_loss_T,
        f"loss_{key}_R": avg_loss_R,
        f"loss_{key}_FL": avg_loss_FL
    }

    return loss_dict


def compute_gravity_camera_loss(
    ...
    pose_encoding_type="absT_quaR_FoV",  # NOTE: this is ignored for gravity camera heads!
    ...
    **kwargs
    ...
    local_pred_pose_encodings = pred_dict['local_pose_enc_list']
    global_pred_pose_encodings = pred_dict['global_pose_enc_list']
    ...
    n_stages = len(local_pred_pose_encodings)

    # Get ground truth camera extrinsics and intrinsics
    gt_g2cs = batch_data['g2cs']
    gt_w2gs = batch_data['w2gs']
    ...
    gt_local_pose_encoding = extri_intri_to_pose_encoding(
        gt_g2cs, gt_intrinsics, image_hw, pose_encoding_type="noT_quaR_FoV"
    )
    gt_global_pose_encoding = extri_intri_to_pose_encoding(
        gt_w2gs, gt_intrinsics, image_hw, pose_encoding_type="absT_quaRy_noFoV"
    )

    local_loss_dict = _gravity_camera_loss_helper(
        "local", local_pred_pose_encodings, gt_local_pose_encoding, valid_frame_mask,
        n_stages, loss_type, gamma, weight_trans, weight_rot, weight_focal,
        pose_encoding_type="noT_quaR_FoV"
    )
    global_loss_dict = _gravity_camera_loss_helper(
        "global", global_pred_pose_encodings, gt_global_pose_encoding, valid_frame_mask,
        n_stages, loss_type, gamma, weight_trans, weight_rot, weight_focal,
        pose_encoding_type="absT_quaRy_noFoV"
    )

    # Return loss dictionary with individual components
    loss_dict = {**local_loss_dict, **global_loss_dict}
    return loss_dict
...
def camera_loss_single(pred_pose_enc, gt_pose_enc, loss_type="l1", pose_encoding_type="absT_quaR_FoV"):
    """
    ...
    Args:
        ...
        pose_encoding_type: One among "absT_quaR_FoV", "noT_quaR_FoV", "absT_quaRy_noFoV"
    Returns:
    ...
    """
    if loss_type == "l1" and pose_encoding_type == "absT_quaR_FoV":
    ...
    elif loss_type == "l1" and pose_encoding_type == "noT_quaR_FoV":
        # No translation component; Rotation: first 4 (quaternion); Focal/Intrinsics: last dims
        loss_T = torch.zeros_like(pred_pose_enc[..., :3])  # Dummy indexing; No translation loss
        loss_R = (pred_pose_enc[..., :4] - gt_pose_enc[..., :4]).abs()
        loss_FL = (pred_pose_enc[..., 4:] - gt_pose_enc[..., 4:]).abs()

    elif loss_type == "l1" and pose_encoding_type == "absT_quaRy_noFoV":
        # Translation: first 3 dims; Rotation (quaty): next 2 dims; No focal/intrinsics component
        loss_T = (pred_pose_enc[..., :3] - gt_pose_enc[..., :3]).abs()
        loss_R = (pred_pose_enc[..., 3:5] - gt_pose_enc[..., 3:5]).abs()
        loss_FL = torch.zeros_like(pred_pose_enc[..., :2])  # Dummy indexing; No focal/intrinsics loss

    elif loss_type == "l2" and pose_encoding_type == "absT_quaR_FoV":
...
def compute_point_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    gt_points_gravity = batch['world_points']
    gt_points_mask = batch['point_masks']

    # scale normalize predicted point (gt gravity points are already normalized)
    pred_points = _normalize_points(pred_points, gt_points_mask)
    ...
    loss_conf, loss_grad, loss_reg = regression_loss(
        pred_points, gt_points_gravity, gt_points_mask, conf=pred_points_conf,
...
def compute_depth_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    # NOTE: we put conf inside regression_loss so that we can also apply conf loss
    # to the gradient loss in a multi-scale manner
...
def point_map_to_normal(point_map, mask, eps=1e-6):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        ...
        left   = pts[:, 1:-1, :-2 , :]   # noqa
        down   = pts[:, 2:,   1:-1, :]   # noqa
        right  = pts[:, 1:-1, 2:,   :]   # noqa

        # Compute direction vectors from center to neighbors
        up_dir    = up    - center  # noqa
        left_dir  = left  - center  # noqa
        down_dir  = down  - center  # noqa
        right_dir = right - center  # noqa
        ...
        v2 = padded_mask[:, 1:-1, :-2 ] & padded_mask[:, 1:-1, 1:-1] & padded_mask[:, 2:,   1:-1]  # noqa
        v3 = padded_mask[:, 2:,   1:-1] & padded_mask[:, 1:-1, 1:-1] & padded_mask[:, 1:-1, 2:]
        v4 = padded_mask[:, 1:-1, 2:  ] & padded_mask[:, 1:-1, 1:-1] & padded_mask[:, :-2,  1:-1]  # noqa
...
def sequence_loss(
    flow_preds, flow_gt, vis, valids, gamma=0.8, vis_aware=False, huber=False,
    delta=10, vis_aware_w=0.1, **kwargs
):
