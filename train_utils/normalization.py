...
def normalize_camera_extrinsics_and_points_batch(
    ...
    if world_points is not None:
        ...
        R = extrinsics[:, 0, :3, :3]
        t = extrinsics[:, 0, :3, 3]
        new_world_points = (world_points @ R.transpose(-1, -2).unsqueeze(1).unsqueeze(2)) + t.unsqueeze(1).unsqueeze(2).unsqueeze(3)  # noqa
    else:
...
def normalize_gravity_camera_extrinsics_and_points_batch(
    c2gs: torch.Tensor,
    g2ws: torch.Tensor,
    cam_points: Optional[torch.Tensor] = None,
    world_points: Optional[torch.Tensor] = None,
    ...
    """
    Normalize camera poses and corresponding 3D points.

    This function transforms the coordinate system to be centered at the first camera
    and optionally scales the scene to have unit average distance.

    Args:
        c2gs: Camera-to-Gravity matrices of shape (B, S, 3, 4)
        g2ws: Gravity-to-World matrices of shape (B, S, 3, 4)
        cam_points: 3D points in camera coordinates of shape (B, S, H, W, 3) or (*,3)
        world_points: 3D points in world coordinates of shape (B, S, H, W, 3) or (*,3)
        depths: Depth maps of shape (B, S, H, W)
        scale_by_points: Whether to normalize the scale based on point distances
    ...
    Returns:
        Tuple containing:
        - Normalized Gravity-to-Camera matrices (g2cs) of shape (B, S, 3, 4)
        - Normalized World-to-Gravity matrices (w2gs) of shape (B, S, 3, 4)
        - Normalized camera points (same shape as input cam_points)
        - Normalized world points (same shape as input world_points)
        - Normalized depths (same shape as input depths)
    """
    # Validate inputs
    check_valid_tensor(c2gs, "c2gs")
    check_valid_tensor(g2ws, "g2ws")
    check_valid_tensor(cam_points, "cam_points")
    check_valid_tensor(world_points, "world_points")
    check_valid_tensor(depths, "depths")
    ...
    g2ws_flat = einops.rearrange(g2ws, "b s r c -> (b s) r c")
    w2gs_flat = closed_form_inverse_se3(g2ws_flat)
    w2gs = einops.rearrange(w2gs_flat, "(b s) r c -> b s r c", b=B, s=S)

    first_cam_g2w = g2ws[:, 0]
    new_w2gs = torch.matmul(w2gs, first_cam_g2w.unsqueeze(1))  # (B, N, 4, 4)

    # g2c is the inverse of c2g (transpose of rotation, translation is 0)
    new_g2cs = torch.zeros_like(c2gs)
    new_g2cs[:, :, :3, :3] = einops.rearrange(c2gs[:, :, :3, :3], "b s r c -> b s c r")
    new_g2cs[:, :, 3, 3] = 1.0

    if world_points is not None:
        first_cam_w2g = w2gs[:, 0]
        R = first_cam_w2g[:, :3, :3]
        t = first_cam_w2g[:, :3, 3]
        new_world_points = (world_points @ R.transpose(-1, -2).unsqueeze(1).unsqueeze(2)) + t.unsqueeze(1).unsqueeze(2).unsqueeze(3)  # noqa
    else:
    ...
    if scale_by_points:
        ...
        new_world_points = new_world_points / avg_scale.view(-1, 1, 1, 1, 1)
        new_w2gs[:, :, :3, 3] = new_w2gs[:, :, :3, 3] / avg_scale.view(-1, 1, 1)
        if depths is not None:
            new_depths = new_depths / avg_scale.view(-1, 1, 1, 1)
        if cam_points is not None:
            new_cam_points = new_cam_points / avg_scale.view(-1, 1, 1, 1, 1)
    else:
        return new_g2cs, new_w2gs[:, :, :3], cam_points, new_world_points, depths

    new_g2cs = check_and_fix_inf_nan(new_g2cs, "new_g2cs", hard_max=None)
    new_w2gs = check_and_fix_inf_nan(new_w2gs, "new_w2gs", hard_max=None)
    new_cam_points = check_and_fix_inf_nan(new_cam_points, "new_cam_points", hard_max=None)
    new_world_points = check_and_fix_inf_nan(new_world_points, "new_world_points", hard_max=None)
    new_depths = check_and_fix_inf_nan(new_depths, "new_depths", hard_max=None)

    return new_g2cs, new_w2gs, new_cam_points, new_world_points, new_depths
