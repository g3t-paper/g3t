...
from train_utils.freeze import freeze_modules  # noqa: E402
from train_utils.logging import setup_logging  # noqa: E402
from train_utils import general as general_train_utils  # noqa: E402
from train_utils.checkpoint import DDPCheckpointSaver  # noqa: E402
from train_utils.optimizer import construct_optimizers  # noqa: E402
from train_utils.distributed import get_machine_local_and_dist_rank  # noqa: E402
...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        general_train_utils.safe_makedirs(self.logging_conf.log_dir)
        setup_logging(
            __name__,
        ...
        general_train_utils.set_seeds(seed_value, self.max_epochs, self.distributed_rank)

        assert general_train_utils.is_dist_avail_and_initialized(), \
        ...
        self.time_elapsed_meter = general_train_utils.DurationMeter("Time Elapsed", self.device, ":.4f")
        ...
        if self.checkpoint_conf.resume_checkpoint_path is not None:
            ...
            ckpt_path = general_train_utils.get_resume_checkpoint(self.checkpoint_conf.save_dir)
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        if self.checkpoint_conf.is_pretrained:

            keys = list(model_state_dict.keys())

            # initialize both local and global camera heads with camera head from the pretrained model
            for key in keys:
                if key.startswith("camera_head."):
                    offset = len("camera_head.")
                    substring = key[offset:]
                    global_camera_idxs = [0, 1, 2, 4, 6]

                    if substring.startswith("pose_branch.fc2.weight") or substring.startswith("pose_branch.fc2.bias"):  # noqa
                        new_key_1 = "local_camera_head." + key[offset:]
                        new_key_2 = "global_camera_head." + key[offset:]
                        local_camera_token = model_state_dict[key].clone()
                        global_camera_token = model_state_dict[key].clone()

                        # ignore translation for local camera token
                        local_camera_token = local_camera_token[3:]

                        # taking T, quat_y and FoV indices for global camera token
                        global_camera_token = global_camera_token[global_camera_idxs]

                        model_state_dict[new_key_1] = local_camera_token
                        model_state_dict[new_key_2] = global_camera_token

                    elif substring.startswith("empty_pose_tokens") or substring.startswith("embed_pose.weight"):
                        new_key_1 = "local_camera_head." + key[offset:]
                        new_key_2 = "global_camera_head." + key[offset:]
                        local_camera_token = model_state_dict[key].clone()
                        global_camera_token = model_state_dict[key].clone()
                        ...
                        new_key_1 = "local_camera_head." + key[offset:]
                        new_key_2 = "global_camera_head." + key[offset:]
                        model_state_dict[new_key_1] = model_state_dict[key]
                        model_state_dict[new_key_2] = model_state_dict[key]
        ...
        if "prev_epoch" in checkpoint:
            self.epoch = checkpoint["prev_epoch"] + 1  # start from the next epoch
    ...
    def _setup_components(self):
        ...
        if self.rank == 0:
            ...
            general_train_utils.model_summary(self.model, log_file=model_summary_path)
    ...
    def save_checkpoint(self, epoch: int, checkpoint_names: Optional[List[str]] = None):
        ...
        general_train_utils.safe_makedirs(checkpoint_folder)
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            general_train_utils.set_seeds(self.seed_value + self.epoch * 100, self.max_epochs, self.distributed_rank)
    ...
    def val_epoch(self, val_loader):
        batch_time = general_train_utils.AverageMeter("Batch Time", self.device, ":.4f")
        data_time = general_train_utils.AverageMeter("Data Time", self.device, ":.4f")
        mem = general_train_utils.AverageMeter("Mem (GB)", self.device, ":.4f")
        data_times = []
        phase = 'val'
        ...
        loss_meters = {
            name: general_train_utils.AverageMeter(name, self.device, ":.4f") for name in loss_names
        }

        progress = general_train_utils.ProgressMeter(
        ...
        for data_iter, batch in enumerate(val_loader):
            ...
            batch = general_train_utils.copy_data_to_device(batch, self.device, non_blocking=True)
    ...
    def train_epoch(self, train_loader):
        ...
        for config in self.gradient_clipper.configs:
            ...
            loss_meters[f"Grad/{param_names}"] = general_train_utils.AverageMeter(
                f"Grad/{param_names}", self.device, ":.4f"
            )
    ...
    def _run_steps_on_batch_chunks(
        ...
        phase: str,
        loss_meters: Dict[str, general_train_utils.AverageMeter],
    ):
        """
    ...
    def _apply_batch_repetition(self, batch: Mapping) -> Mapping:
        ...
        for key in tensor_keys:
            if key in batch:
                ...
                batch[key] = torch.concatenate(
                    [original_tensor, torch.flip(original_tensor, dims=[1])], dim=0
                )
    ...
    def _process_batch(self, batch: Mapping):
        # NOTE: normalization code is moved to loss.py
...
def chunk_batch_for_accum_steps(batch: Mapping, accum_steps: int) -> List[Mapping]:
    ...
    if accum_steps == 1:
        return [batch]
    if accum_steps > batch["images"].shape[0]:
        return [batch]  # cannot split, return the original batch
