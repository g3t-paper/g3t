...
def depth_to_world_coords_points(
    ...
    Args:
        ...
        extrinsic (np.ndarray): Camera extrinsic matrix of shape (3, 4).
            OpenCV camera coordinate convention, cam from world.
...
def make_4x4(pose_3x4):
    """
    Given a torch tensor of shape (..., 3, 4), return a tensor of shape (..., 4, 4)
    where the last row is (0,0,0,1).
    """
    assert pose_3x4.shape[-2:] == (3, 4)
    shape = pose_3x4.shape[:-2] + (4, 4)
    pose_4x4 = torch.eye(4, device=pose_3x4.device).expand(shape).clone()
    pose_4x4[..., :3, :4] = pose_3x4
    return pose_4x4


def make_4x4_np(pose_3x4):
    """
    Given a numpy array of shape (..., 3, 4), return an array of shape (..., 4, 4)
    where the last row is (0,0,0,1).
    """
    assert pose_3x4.shape[-2:] == (3, 4)
    shape = pose_3x4.shape[:-2] + (4, 4)
    pose_4x4 = np.broadcast_to(np.eye(4), shape).copy()
    pose_4x4[..., :3, :4] = pose_3x4
    return pose_4x4
